...
Else {
    Write-Output "Script is already running in another instance. Waiting..." -ForegroundColor Red
    Do {
        test-path $prop.lock_path > $null
        Start-Sleep 10
    }
    Until (-Not (Test-Path $prop.lock_path))
    Write-Output "Other instance ended. Continuing..." -ForegroundColor Green
    New-Item $prop.lock_path -Force
}
Clear-Host