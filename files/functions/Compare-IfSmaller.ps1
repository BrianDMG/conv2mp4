...
Function Compare-IfSmaller {
  ...
  Try {
    ...
    If ($deltaLT -lt 1) {
      ...
      Add-Log "$($time.Invoke()) New file is $($deltaLT_KB)KB smaller."
    }
    Elseif ($deltaLT -lt -1024) {
      $deltaLT_GB = ($deltaLT / 1024)
      $deltaLT_GB = [Math]::Round($deltaLT_GB, 2)
      Add-Log "$($time.Invoke()) New file is $($deltaLT_GB)GB smaller."
    }
    Else {
      Add-Log "$($time.Invoke()) New file is $($deltaLT)MB smaller."
    }
    ...
    If ($script:diskUsageDelta -gt -1 -AND $script:diskUsageDelta -lt 1) {
      ...
      Add-Log "$($time.Invoke()) Current cumulative storage difference: $($diskUsageDelta_KB)KB"
    }
    Elseif ($script:diskUsageDelta -lt -1024 -OR $script:diskUsageDelta -gt 1024) {
      ...
      Add-Log "$($time.Invoke()) Current cumulative storage difference: $($diskUsageDelta_GB)GB"
    }
    Else {
      $script:diskUsageDelta = [Math]::Round($script:diskUsageDelta, 2)
      Add-Log "$($time.Invoke()) Current cumulative storage difference: $($script:diskUsageDelta)MB"
    }
  }
  Catch {
