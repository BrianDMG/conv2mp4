...
If ($cfg.logging.use_ignore_list) {
  ...
  $fileList = Get-ChildItem "$((Get-Item -Path $cfg.paths.media_path).FullName)" -Include ( $cfg.conversion.include_file_types -split ',' ).trim() -Exclude $(Get-Content $prop.paths.files.ignore) -Recurse |
...
Else {
  $fileList = Get-ChildItem "$((Get-Item -Path $cfg.paths.media_path).FullName)" -Include ( $cfg.conversion.include_file_types -split ',' ).trim() -Recurse |
    ForEach-Object {$counter = 1} {
      Write-Progress -Activity "Found $($counter) file(s) so far..." -Status "Processing..." -CurrentOperation "$($_.FullName)"
      $_
      $counter++
