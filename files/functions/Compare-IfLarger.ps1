...
Function Compare-IfLarger {
  ...
  Try {
    ...
    If ($deltaGT -lt 1) {
      ...
      Add-Log "$($time.Invoke()) New file is $($deltaGT_KB)KB larger."
    }
    Elseif ($deltaGT -gt 1024) {
      $deltaGT_GB = ($deltaGT / 1024)
      $deltaGT_GB = [Math]::Round($deltaGT_GB, 2)
      Add-Log "$($time.Invoke()) New file is $($deltaGT_GB)GB larger."
    }
    Else {
      Add-Log "$($time.Invoke()) New file is $($deltaGT)MB larger."
    }
