...
Function Convert-File {
  ...
  If ($ConvertType -eq "Handbrake") {
    ...
    Try {
      $hbCMD
      Add-Log "$($time.Invoke()) Handbrake finished."
    }
    Catch {
      Add-Log "$($time.Invoke()) ERROR: Handbrake has encountered an error."
      Add-Log $_
    }
  }
  Else {
    ...
    If ($KeepSubs) {
      ...
      Else {
        Add-Log "$($time.Invoke()) Detected bitmap subtitles, not keeping subtitles."
